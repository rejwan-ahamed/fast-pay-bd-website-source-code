...
if (isset($_POST['update'])) {
    ...
    // INSERT INTO `state` (`sno`, `state`) VALUES (NULL, 'online');

    $state = $_POST['state'];

    $sql = "UPDATE `state` SET `state`='$state' WHERE sno ='1' ";
    $query = mysqli_query($con, $sql);

    if ($query) {
        $up = '<div class="alert alert-primary" role="alert">
        state updated successfully
      </div>';
    } else {
        echo "state not updated";
    }
}
                    ...
                    <section class="register text-center">
                        <main class="form-signin">
                            <form action="#" method="POST">
                                ...
                                <select class="form-select" id="country" required="" name="state">
